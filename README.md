...
而使用组件 因为是插入表中 所以在事务里提交创建异步命令和订单肯定是原子性的 然后在异步命令里发送mq消息 就不需要特意使用mq的事务型消息 如果发送失败 会进行重试 可以不用为每个想高可靠的消息写业务回调接口 而且目前我了解的mq只有rocketmq在这方面做的比较好 rabbitmq Kafka ActiveMQ的事务型消息处理起来更加麻烦
