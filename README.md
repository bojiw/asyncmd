...
```
    <bean id="asynGroupConfig" class="com.asyncmd.config.AsynGroupConfig">
        ...
        <!--非必填项-->
        <!-- 调度中心调度执行相关配置 start -->
        <!-- 从数据库捞取未执行的异步命令多久捞取一次 默认是2秒捞取一次  0/3 * * * * ?代表3秒捞取一次-->
...
压测结束以后 每张表大概2千条命令 4张表 总共挤压大概8千条命令

当qps达到500 则会开始出现挤压情况 总生产6万条数据
2分钟以后每张表挤压5千多条命令 4张表达到2万多条命令 

消费挤压命令逻辑为
每隔2秒 就会从命令表中捞取30条数据 4张表 30*4=120 代表2秒钟处理120数据 2万条数据大约需要处理3分钟 如果是分成8张表则8*30=240 每隔两秒处理240条数据
